...
## Final PR merge checklist (to avoid losing fixes)

When GitHub shows merge conflicts in `app.js`, do **not** click only "Accept current changes" for the online sync blocks.

Use this flow:

1. Open conflict editor for `app.js`.
2. Keep the incoming/fixed logic for these functions:
   - `normalizeList`
   - `normalizeCardList`
   - `normalizePlayer`
   - `serializeGameState`
   ...
   - `maybePlaySyncedShow`
3. Ensure `applyRemoteGameState` normalizes:
   - `players` via `.map(normalizePlayer)`
   - `drawPile` via `normalizeCardList`
   - `discardPool` via `normalizeCardList`
4. Merge PR.
5. Deploy again: `firebase deploy`.
6. Hard-refresh all players (`Ctrl+F5` / `Cmd+Shift+R`).

### How to confirm you are on the latest build

`index.html` now loads versioned module URLs (`?v=20260219a`) so GitHub Pages caches refresh to the latest JS after merge/deploy.


### Show result behavior (online)

- Show popup now appears for **all connected players**.
